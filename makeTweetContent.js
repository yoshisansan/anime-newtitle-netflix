...
const getMessage = {
    MessageAsync: async (animeTitle) => {
        ...
        if(nextTweet.length > 0) {
            ...
            await getMessage.BridgeToTwitter(messages);
        }
    },
    ...
    MessageFunc: async (animeTitle) => {
        ...
        message = [];
        console.log(`makeTweetContent${messages}`);
    },

    BridgeToTwitter: async (message) => {
        await twitter.twitterInitialize(messages);
        return;
    }
}
